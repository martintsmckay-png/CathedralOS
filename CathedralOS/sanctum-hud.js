...
export class SanctumHUD {
  ...
  ingestStream(rawText, sourceId = "7-G_SECTOR_LEAK") {
    ...
    // Blast Shield Protection: Use optional chaining to guarantee counts evaluate to 0 if null
    const jsCount = relics.jsSnippets?.length ?? 0;
    const htmlCount = relics.htmlSnippets?.length ?? 0;
    const shellCount = relics.shellCommands?.length ?? 0;

    this.totalRelics += (jsCount + htmlCount + shellCount);
