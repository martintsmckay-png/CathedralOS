...
def save_artifact_tags(artifact_name, tags):
    TAG_DIR.mkdir(parents=True, exist_ok=True)
    tag_path = TAG_DIR / f"{artifact_name}.json"

    tags["updated"] = datetime.now(timezone.utc).isoformat()
...
def tag_artifact(artifact_path):
    print(Color.wrap(f">> TAG ARTIFACT - {artifact_path.name}", Color.CYAN))
    print()

    tags = load_artifact_tags(artifact_path.name)

    print(Color.wrap("CURRENT TAGS:", Color.BLUE))
    print(f"Notes: {tags.get('notes') or '(none)'}")
    print(f"Categories: {', '.join(tags.get('categories', [])) or '(none)'}")
    print(f"Flags: {', '.join(tags.get('flags', [])) or '(none)'}")
    print(f"Last Updated: {tags.get('updated') or '(never)'}")
    print()
    ...
    while True:
        ...
        if action == "1":
            tags["notes"] = input("Enter new notes: ").strip()
            save_artifact_tags(artifact_path.name, tags)
            print(Color.wrap("Notes updated.", Color.GREEN))
            print()
        elif action == "2":
            category = input("Enter category: ").strip()
            if category:
                tags.setdefault("categories", []).append(category)
                save_artifact_tags(artifact_path.name, tags)
                print(Color.wrap("Category added.", Color.GREEN))
                print()
        elif action == "3":
            flag = input("Enter flag: ").strip()
            if flag:
                tags.setdefault("flags", []).append(flag)
                save_artifact_tags(artifact_path.name, tags)
                print(Color.wrap("Flag added.", Color.GREEN))
                print()
        elif action == "4":
            tags = {
                "notes": "",
            ...
            save_artifact_tags(artifact_path.name, tags)
            print(Color.wrap("All tags cleared.", Color.YELLOW))
            print()
        elif action == "5":
            return
        else:
